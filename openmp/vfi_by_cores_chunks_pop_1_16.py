...
#    list_chunks      = np.arange(100,501,100)
    ...
    for num_cores in list_cores:
        for na in list_chunks:
            for num_runs in range(nrun):
                ...
                print("####### ", num_runs+1, " times run with ",num_cores, " cores and ", na, "chunks  #######")
    ...
    with open("runtime_vfi_by_cores_chunks_pop_1_16.txt","w+") as outfile:
        json.dump(data, outfile)
    
    
    # Retrieve the results from the output file
    with open("runtime_vfi_by_cores_chunks_pop_1_16.txt") as json_data:
