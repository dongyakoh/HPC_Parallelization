...
#    print(" ")
    ...
    try:
        ...
        chunk_size  = int(hh.na*hh.ne // (nc*num_cores))
        ...
        for age in reversed(range(hh.T)):
            ...
            # GRID SEARCH AT AGE < T
                ...
                results = pool.map(vfi_opt_partial, np.arange(hh.na*hh.ne),chunk_size)
...
#    list_cores      = np.arange(2,9,1)
    ...
    # Number of cores
#    list_chunks      = np.arange(100,501,100)
    ...
    for num_cores in list_cores:
        for nc in list_chunks:
            for num_runs in range(nrun):
                
                results  = run_vfi_opt(num_cores, nc)
                ...
                print("####### ", num_runs+1, " times run with ",num_cores, " cores and ", nc, "chunks  #######")
    ...
    with open("runtime_vfi_by_cores_chunks_oop_1_32.txt","w+") as outfile:
        json.dump(data, outfile)
    
    
    # Retrieve the results from the output file
    with open("runtime_vfi_by_cores_chunks_oop_1_32.txt") as json_data:
        d = json.load(json_data)
        pprint(d)
    for ik in range(len(d)):
        print(d[ik]['core'],d[ik]['chunk'],d[ik]['run'],d[ik]['age'],d[ik]['runtime'])
