...
#            comm.Bcast([V1,(hh.na,hh.ne),MPI.DOUBLE],root=0)
            ...
            if hh.na*hh.ne < ub:
                ub = hh.na*hh.ne
            leng = ub - lb
#            disp = np.zeros(comm.size)
#            count = np.zeros(comm.size)
#            for ik in range(comm.size):
#                disp[ik] = (ik*leng,3)
#                count[ik] = (leng,3)
...
#                comm.Allgather([Vp, (int(leng),3), MPI.DOUBLE], [results, (hh.na,hh.ne,3), MPI.DOUBLE])
